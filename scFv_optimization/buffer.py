...
class ReplayBuffer:
    ...
    def __init__(self) -> None:
        ...
        self.data: dict[str, list[T.Tensor]] = {"states": [], "action1s": [], "action2s": [], 
    ...
    def _calculate_returns(self, rewards: T.Tensor) -> T.Tensor:
        ...
        for n in range(config.N_PARALLELS):
            ...
            ret = 0
            for t in reversed(range(config.PPO_TIME_HORIZON)):
                ret = rewards[n, t] + config.PPO_GAMMA * ret
                returns[n, t] = ret
    ...
    def _calculate_gae(self, rewards: T.Tensor, pred_values: T.Tensor) -> T.Tensor:
        ...
        for t in reversed(range(config.PPO_TIME_HORIZON)):
            delta = rewards[:, t] + config.PPO_GAMMA * pred_values[:, t + 1] - pred_values[:, t]
            next_advantages = delta + config.PPO_GAMMA * config.PPO_LAMBDA * next_advantages
            advantages[:, t] = next_advantages

        return advantages.flatten()
    
    def get_train_data(self) -> \
            tuple[T.Tensor, T.Tensor, T.Tensor, T.Tensor, T.Tensor, T.Tensor, T.Tensor]:
    ...
    def get_batch_indices(self) -> list[list[int]]:
        ...
        batch_indices = [
            all_indices[i:i + config.PPO_BATCH_SIZE]
            for i in range(0, n_data, config.PPO_BATCH_SIZE)]
