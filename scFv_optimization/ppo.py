...
class PPO:
    ...
    def choose_actions(self, states: T.Tensor) -> \
            tuple[T.Tensor, T.Tensor, T.Tensor, T.Tensor, T.Tensor]:

        with T.no_grad():

            probs: T.Tensor = self.actor1(states)
            dists = Categorical(probs.cpu())
            action1s: T.Tensor = dists.sample()
            log_prob1s: T.Tensor = dists.log_prob(action1s)

            action1s_ = action1s.to(DEVICE)
            probs: T.Tensor = self.actor2(states, action1s_)
            dists = Categorical(probs.cpu())
            action2s: T.Tensor = dists.sample()
            log_prob2s: T.Tensor = dists.log_prob(action2s)
    ...
    def learn(self) -> tuple[float, float, float, float, float]:
        ...
        states, action1s, action2s, old_log_prob1s, old_log_prob2s, returns, gaes = \
            self.buffer.get_train_data()

        for _ in range(config.PPO_N_EPOCHS):
            for batch_indices in self.buffer.get_batch_indices():
                ...
                surrogate1 = T.clamp(ratio1, 1 - config.PPO_EPSILON, 1 + config.PPO_EPSILON)
                ...
                surrogate2 = T.clamp(ratio2, 1 - config.PPO_EPSILON, 1 + config.PPO_EPSILON)
                ...
                entropy_bonus = config.PPO_BETA * (entropy_bonus1 + entropy_bonus2)
